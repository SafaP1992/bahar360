...
$wsdl_url = "https://pec.shaparak.ir/NewIPGServices/MultiplexedSale/OnlineMultiplexedSalePaymentService.asmx?WSDL";
...
try {
	...
	if ($result->MultiplexedSalePaymentRequestResult->Token && $result->MultiplexedSalePaymentRequestResult->Status === 0) {
		...
		$err_msg = "(<strong> کد خطا : " . $result->MultiplexedSalePaymentRequestResult->Status . "</strong>) " . $result->MultiplexedSalePaymentRequestResult->Message . "</div>";
	}
} catch (Exception $ex) {
